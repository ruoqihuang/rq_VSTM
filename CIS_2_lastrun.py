...
expName = 'CIS_2'  # from the Builder filename that created this script
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='/Users/athenahuang/Downloads/CIS_2_lastrun.py',
...
textWelcome = visual.TextStim(win=win, name='textWelcome',
    text='Welcome to the short term memory experiment! Please press space to enter.',
    font='Open Sans',
...
textInstruction = visual.TextStim(win=win, name='textInstruction',
    text='You will be presented a series of image.\n\nAfter each imgae, select the one that you think matches the one shown.\n\nPress space to continue.',
    font='Open Sans',
    pos=(0, 0), height=0.1, wrapWidth=None, ori=0.0, 
    color='white', colorSpace='rgb', opacity=None, 
    languageStyle='LTR',
...
imageTraining = visual.ImageStim(
    win=win,
    name='imageTraining', 
    image='sin', mask=None,
...
imageChoice1 = visual.ImageStim(
    win=win,
    name='imageChoice1', 
    image=Choice1, mask=None,
...
WelcomeScreenComponents = [textWelcome, key_resp]
...
_key_resp_2_allKeys = []
# keep track of which components have finished
InstructionsComponents = [textInstruction, key_resp_2]
...
trialsTraining = data.TrialHandler(nReps=1.0, method='random', 
    extraInfo=expInfo, originPath=-1,
    trialList=data.importConditions('CIS Experiment/image_stimuli.xlsx', selection='0:6'),
...
for thisTrialsTraining in trialsTraining:
    ...
    continueRoutine = True
    # update component parameters for each repeat
    imageTraining.setImage(ImageFile)
    key_resp3.keys = []
    key_resp3.rt = []
    _key_resp3_allKeys = []
    # keep track of which components have finished
    TrainingComponents = [imageTraining, key_resp3, imageChoice1]
    ...
    while continueRoutine:
        ...
        # *imageTraining* updates
        if imageTraining.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            imageTraining.frameNStart = frameN  # exact frame index
            imageTraining.tStart = t  # local t and not account for scr refresh
            imageTraining.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(imageTraining, 'tStartRefresh')  # time at next scr refresh
            imageTraining.setAutoDraw(True)
        if imageTraining.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > imageTraining.tStartRefresh + 0.5-frameTolerance:
                # keep track of stop time/frame for later
                imageTraining.tStop = t  # not accounting for scr refresh
                imageTraining.frameNStop = frameN  # exact frame index
                win.timeOnFlip(imageTraining, 'tStopRefresh')  # time at next scr refresh
                imageTraining.setAutoDraw(False)
        ...
        if key_resp3.status == STARTED and not waitOnFlip:
            theseKeys = key_resp3.getKeys(keyList=['1', '2', '3', '4', '5', '6'], waitRelease=False)
        ...
        # *imageChoice1* updates
        if imageChoice1.status == NOT_STARTED and tThisFlip >= 1.25-frameTolerance:
            # keep track of start time/frame for later
            imageChoice1.frameNStart = frameN  # exact frame index
            imageChoice1.tStart = t  # local t and not account for scr refresh
            imageChoice1.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(imageChoice1, 'tStartRefresh')  # time at next scr refresh
            imageChoice1.setAutoDraw(True)
    ...
    for thisComponent in TrainingComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    trialsTraining.addData('imageTraining.started', imageTraining.tStartRefresh)
    trialsTraining.addData('imageTraining.stopped', imageTraining.tStopRefresh)
    # check responses
    if key_resp3.keys in ['', [], None]:  # No response was made
        key_resp3.keys = None
    trialsTraining.addData('key_resp3.keys',key_resp3.keys)
    if key_resp3.keys != None:  # we had a response
        trialsTraining.addData('key_resp3.rt', key_resp3.rt)
    trialsTraining.addData('key_resp3.started', key_resp3.tStartRefresh)
    trialsTraining.addData('key_resp3.stopped', key_resp3.tStopRefresh)
    trialsTraining.addData('imageChoice1.started', imageChoice1.tStartRefresh)
    trialsTraining.addData('imageChoice1.stopped', imageChoice1.tStopRefresh)
...
trials = data.TrialHandler(nReps=1.0, method='random', 
    extraInfo=expInfo, originPath=-1,
    trialList=data.importConditions('CIS Experiment/image_stimuli.xlsx'),
...
for thisTrial in trials:
    ...
    _key_resp_3_allKeys = []
    # keep track of which components have finished
    StudyTrialsComponents = [image, key_resp_3]
    ...
    t = 0
    _timeToFirstFrame = win.getFutureFlipTime(clock="now")
    StudyTrialsClock.reset(-_timeToFirstFrame)  # t0 is time of first possible flip
    frameN = -1
    ...
    while continueRoutine:
        ...
        tThisFlip = win.getFutureFlipTime(clock=StudyTrialsClock)
        tThisFlipGlobal = win.getFutureFlipTime(clock=None)
        frameN = frameN + 1  # number of completed frames (so 0 is the first frame)
        # update/draw components on each frame
    ...
    # the Routine "StudyTrials" was not non-slip safe, so reset the non-slip timer
    routineTimer.reset()
    thisExp.nextEntry()
